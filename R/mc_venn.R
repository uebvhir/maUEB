#' Venn Diagram or UpsetR plot of common/unique features between multiple gene lists
#'
#' Creates a Venn Diagram (for up to 5 comparisons) or UpsetR plot (>5 comparisons) of the common/unique features selected in multiple comparisons. The lists of common/unique features between comparisons are saved in an excel file.
#' @param listofcsv_venn List of toptables to summarize
...
mc_venn <- function(listofcsv, listofcoef, volc_logFC=rep(1,length(listofcoef)), volc_pval=c(rep("adj.P.Val", length(listofcoef))), volc_pval.thr=rep(0.05, length(listofcoef)), volc_x0=rep(-3, length(listofcoef)), volc_x1=rep(+3, length(listofcoef)), volc_y0=rep(0, length(listofcoef)), volc_y1=rep(10, length(listofcoef)), n=1, cols=1, outputDir, label=""){
    ...
    for(i in 1:length(listofcsv)){
        ...
        g <- ggplot(data = top,
            ...
            scale_colour_manual(values = c("lightblue", "purple"), breaks="TRUE", labels=paste0(volc_pval[i], "<",volc_pval.thr[i], " & abs(logFC)>", volc_logFC[i]), name="Threshold") +
