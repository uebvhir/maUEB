...
qc_boxplot <- function(data, group="Group", group.color="Colors", samplenames="ShortName", outputDir, bp.main="Boxplot for array intensity", label, cex.axis=0.5, bp.ylab="log2 intensity", cex.lab=0.7, las=2, bp.legend=TRUE, bp.legend.posx=10, bp.legend.posy=13, bp.legend.cex=0.6, which="all",...){
    ...
    oligo::boxplot(x=data, col=col, cex.axis=cex.axis, las=las, ylab=bp.ylab, cex.lab=cex.lab,
                   names=names, which=which, main=paste0(bp.main, ". ", label))
