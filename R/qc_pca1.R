...
#' @param dim A vector with the number of dimensions (2 and/or 3) to project the PCA. Defaults to 2.
...
qc_pca1 <- function(data, scale, dim=2, pca2D_factors=c("Group"), pca3D_factors=c("Group"), targets, col.group="Colors", colorlist=NULL, names, batchRemove=FALSE, batchFactors=NULL, outputDir, label, size = 1.5, glineas = 0.25){
    ...
    if (3%in%dim){
        ...
        listplots3D <- lapply(pca3D_factors, function(fact3d){
            ...
            p <- plot_ly(dataDf, type = "scatter3d", x = dataDf$PC1 , y = dataDf$PC2, z= dataDf$PC3,
                    text = rownames(dataDf), mode = "markers+text", showlegend =TRUE,
                    color = ~targets[,fact3d], colors = pcacolors) %>%
                layout(title=pcatitle,
                       scene = list(xaxis = list(title = paste("PC1", loads[1], "%")),
                                    yaxis = list(title = paste("PC2", loads[2], "%")),
                                    zaxis = list(title = paste("PC3", loads[3], "%"))))
