...
mc_all <- function(listofcsv, namescomp, label, colFeat, colPval, pval, colFC, FC, pltR=TRUE,
    ...
    if (Venn_plots){listsharedelems <- lapply(seq_along(venn_comparNames), function(v) {
        setwd(resultsDir)
        namescomp <- names(listofTopNamed)[venn_compar[[v]]]
        mc_venn_upset(listofTopNamed=listofTopNamed, namescomp=namescomp, label = venn_comparNames[v], colFeat = colFeat[v],
