...
#' @examples
#' createBoxplot(data=rawData, group=targets$Group, col=targets$Colors, names=targets$ShortName, resultsDir, label="RawData")
...
createBoxplot <- function(data, group, col, names, resultsDir, label, cex.axis=0.5, cex.lab=0.7, las=2, plotlegend=TRUE, legend.posx=10, legend.posy=13, legend.cex=0.6, ...){
    ...
    oligo::boxplot(data, which="all", col=col, cex.axis=cex.axis, las=las, ylab="log2 intensity", cex.lab=cex.lab,
    ...
    pdf(file.path(resultsDir, paste0("Boxplot", label, ".pdf")))
    oligo::boxplot(data, which="all", col=col, cex.axis=cex.axis, las=las, ylab="log2 intensity", cex.lab=cex.lab,
                   names=names, main=paste0("Boxplot for array intensity: ", label))
    if (plotlegend) {legend(x=legend.posx, y=legend.posy, legend=unique(group), fill=unique(col), cex=legend.cex, bg="white")}
    dev.off()
