...
<!-- ```{r} -->
<!-- #Save the annotation for filtered genes -->
<!--     if (save_annot_filt){ -->
<!--         at <- aafTableAnn(rownames(eset_filtered), annotPackage) -->
<!--         saveHTML (at, file.path(resultsDir, "Annotations.Filtered.html"), -->
<!--                   "Annotations for filtered genes") -->
<!--         saveText (at, file.path(resultsDir, "Annotations.Filtered.csv"), header=TRUE) -->
<!--     } -->
<!-- ``` -->
...
Function `dea_toptab` allows to obtain the toptables for the comparisons analyzed. To create html tables use `html_report`=TRUE (ojo q triga molt).
...
<!-- ```{r} -->
<!-- dea_plotvolcano(listofcsv, volc_pval, volc_pval.thr, volc_logFC, volc_x0, volc_x1, volc_y0, volc_y1, numplots=6, label, outputDir=resultsDir) -->
<!-- ``` -->


### All DEA in one function

All the steps performed in `Block 02- DEA` can be executed in a single step using `dea_all()` function. The result is a list with objects corresponding to each step of the analysis: design, fit, and fit.main.

```{r}
dea.reslist <- dea_all(data=eset_analysis, targets=pData(eset_analysis), sampleNames="ShortName", group=design.mainfact, covariates=design.cofact, fmla=NULL, block.cor=blocking.fact, contrastsv=contrastsv, moderated=TRUE, summaryFN=resultsSummFN, outputDir=resultsDir)
```

```{r}
dea.reslist$design
dea.reslist$fit.main$contrasts
#extract object fit.main for further analysis:
fit.main <- dea.reslist$fit.main
```

Afegir ngc, toptables i volcanoplots en la mateixa funcio dea_all
