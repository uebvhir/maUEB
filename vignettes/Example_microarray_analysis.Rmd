...
Builds a targets template dataframe and saves it into a ;-separated file (csv file: _targets.RSRCHR.STUDY.template.csv_). First column (`FileName`) contains the names of the cel files contained in the celfiles directory specified in parameters. The remaining columns correspond to the descriptors specified in global parameters, among which there will be the following: `Group`,  `ShortName`, and `Colors`. Other variables may be `Batch`, `PatientID`, or other variables of interest. Then, the information for each descriptor needs to be filled manually in excel/libreoffice and saved as csv file using ; separator (file: _targets.RSRCHR.STUDY.csv_).
...
This function reads the targets file (_targets.RSRCHR.STUDY.csv_) generated in section \@ref(buildTargets). The cel file names will be set as rownames.
...
Read the cel files listed in targets rownames (and in the same order) and create an ExpressionSet of raw data.
